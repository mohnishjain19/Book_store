...
const addBook =async(req,res,next)=>{
    const{img,name,author,description,price,available}=req.body;
    let book;
    try
    {
            book=new Book({
...
const updateBook=async(req,res,next) =>{
    ...
    if(!book)
    {
        return res.status(404).json({message:"Unable to Update by his id"});
    }
    return res.status(200).json({book});
}
const deleteBook=async(req,res,next)=>{
    ...
    if(!book)
    {
        return res.status(404).json({message:"Unable to Delete by his id"});
    }
